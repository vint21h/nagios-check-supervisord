...
VERSION=`python -c "import check_supervisord; print(check_supervisord.__version__);"`
...
test:
	py.test -p no:cacheprovider -v tests --cov=check_supervisord --verbose --color=yes;\
